      ...
      <div className="bg-white rounded-2xl shadow-2xl w-[380px] md:w-[420px] h-[600px] flex flex-col overflow-hidden border-2 border-blue-900 animate-slideUp">
        ...
        <div className="bg-gradient-to-r from-blue-900 to-blue-800 text-white p-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="p-2 hover:bg-white hover:text-blue-900 rounded-full transition-all duration-200 bg-blue-800"
              title="Minimize"
            >
        ...
        {messages.length <= 2 && !leadCaptured && (
          <div className="p-3 bg-white border-t border-gray-200">
            <p className="text-xs text-gray-700 mb-2 font-bold">Choose a service:</p>
            <div className="flex flex-col gap-2">
              {mainOptions.map((option, index) => (
                <button
                  key={index}
                  onClick={() => handleQuickReply(option)}
                  className="px-4 py-2.5 bg-gradient-to-r from-blue-900 to-blue-800 hover:from-blue-800 hover:to-blue-700 text-white text-sm font-semibold rounded-lg transition-all transform hover:scale-105 shadow-md"
                >
                  {option}
                </button>
              ))}
            ...
            <div className="grid grid-cols-2 gap-2">
              {actionButtons.map((btn, index) => (
                <button
                  key={index}
                  onClick={btn.action}
                  className="px-3 py-2 bg-white hover:bg-gray-100 text-blue-900 text-xs font-medium rounded-lg border border-gray-300 transition-colors"
                >
                  {btn.text}
                </button>
              ))}
